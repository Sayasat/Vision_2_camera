...
cam = Gemini335LECamera(serial_number="CPE345P0004A")
cam.start()
...
while True:
    ...
    if focus:
        ...
        # --- ДОБАВЬТЕ ЭТО ДЛЯ ПРОВЕРКИ ГРАНИЦ ---
        if safe_contour is not None:
            # Рисуем внутреннюю "безопасную зону" (синим), где ищутся объекты
            cv2.drawContours(color, [safe_contour], -1, (255, 0, 0), 2)
        # ---
    ...
    #             # robot.safe_move_xyz(120,-220,0,0,90)
    #             # robot.safe_move_xyz(120,-200,-250,0,90)
    #             # robot.vacuum_off()
    #             # time.sleep(0.5)
    #             # print("XYZ camera:", cam_point) 
    #             # print("XYZ robot:", pick_pos)

    #             # print(
    #             #     f'{{ "cam": [{cx:.3f}, {cy:.3f}, {cz:.3f}], '
    #             #     f'"rob": [{rx:.3f}, {ry:.3f}, {rz:.3f}] }},'
    #             # )
